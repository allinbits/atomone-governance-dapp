...
export const useValidators = (height?: string) => {
  const validatorList = height
    ? getValset(height)
    : getValidators();
  const validators = computed(() => {
    if (validatorList.value) {
      return validatorList.value.proposal_validator_status_snapshot;
    } else {
      return [];
    }
  });
  const getVotingPower = async (address: string) => {
    ...
    const delegated = height
      ? await getDelegatedAsync(
        address,
        parseInt(height)
      )
      : await getDelegatedAsync(address);
    if (delegated && delegated.staked_balances && delegated.staked_balances.length > 0) {
      return delegated.staked_balances.reduce(
        (total, stakedBalance) => {
